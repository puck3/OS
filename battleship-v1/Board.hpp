...
class Board {
 ...
 public:
  ...
  void throw_if_invalid_ship(int length, int x1, int y1, int x2, int y2) const {
    ...
    if (!((x2 - x1 == length - 1 && y1 == y2) ||
          (y2 - y1 == length - 1 && x1 == x2))) {
  ...
  void add_ship(int length) noexcept {
    ...
    while (true) {
      ...
      try {
        if ((length == 1 && pos.size() != 2) ||
            (length != 1 && (pos1.size() != 2 || pos2.size() != 2))) {
  ...
  Board()
      : main_board(std::vector<std::vector<cell_t>>(
            SIZE, std::vector<cell_t>(SIZE, empty))),
        hit_board(std::vector<std::vector<cell_t>>(
            SIZE, std::vector<cell_t>(SIZE, empty))),
        points(0) {}

  virtual ~Board() = default;

  bool check_win() const noexcept { return points == 20; }
